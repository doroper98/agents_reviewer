...
ssot_for:
  ...
  - "셀 수 있는 단위 어휘 — 칸 질감·캡슐·잉크 사다리·읽는 법 캡션 (§10.1, v8.6.0)"
...
③ **7단 사다리는 순위 전용, 구성은 4단.** 순위 비교(bar·lollipop·scatter 등 항목이
5~7개인 곳)는 `[1, .78, .60, .44, .30, .20, .12]` 7단, 구성비(donut·stacked)는 기존
`[1, .42, .24, .13]` 4단을 쓴다. 8개 이상은 1 → .12 선형. 어느 쪽이든 hue 는 쓰지
않는다 (§10 계승). 액센트는 차트당 1요소 — 보통 1위 값 라벨 하나.
