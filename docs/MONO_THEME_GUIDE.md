...
ssot_for:
  ...
  - "셀 수 있는 단위 어휘 — 칸 질감·캡슐·액센트 농도 사다리·읽는 법 캡션 (§10.1, v8.6.0 · 사다리 색 v8.6.5 · 링·축·단계·글리프 v8.7.0)"
...
않는다. 같은 게이트가 v8.7.0 의 `stacked.texture:"rung"`(세로 칸 적층)에도 붙는다 —
시나리오 이름 ≤6자 · 시나리오 ≤8, 넘으면 기본 가로 적층으로 강등.

### v8.7.0 어휘 확장 — 링·축·단계·글리프

앞의 ①~⑦ 을 새 자리 네 곳으로 넓힌 것이다. 새 문법이 아니라 *같은 문법의 적용처* 다.

⑧ **반원 100 눈금 링 = 목표 하나** (`gauge`). 눈금 100개가 곧 목표라, 채워진 눈금이
달성률이고 남은 눈금이 갈 길이다. 채운 눈금은 액센트 불투명도 1, 안 채운 눈금은
`LADDER_MIN`(.16) — 같은 사다리의 양 끝이다. 목표를 넘으면 링을 부풀리지 않고 초과분을
링 *바깥* 에 따로 그린다(값을 왜곡하지 않기). 중앙 큰 숫자는 §3.3 대로 `--text` 이고
액센트를 쓰지 않는다. 읽는 법 캡션은 "눈금 하나 = 목표의 1% · 목표까지 n눈금".

⑨ **양극 축 hairline = 위치, 점 크기 = 초점** (`spectrum`). 축은 `--text` .35 의 실선
하나에 양 끝·중앙 눈금뿐이고, 값 눈금도 값 라벨도 두지 않는다 — 값이 수량이 아니라
*위치* 이기 때문이다(-1 왼쪽 끝 ~ +1 오른쪽 끝). 점은 액센트 하나에 초점만 r7·불투명도
1, 나머지는 r4·.55 (hue 로 나누지 않는다). 라벨은 좌→우로 훑으며 두 층으로 비껴 앉히고
연결선을 남긴다 (CHART-AP-33 회피 방식 계승). 축 끝 라벨은 caps·자간 .08em·`--muted`.

⑩ **단계 폭은 눈금 수가 만든다** (`funnel`). 사다리꼴을 그려 폭을 면적으로 주지 않고,
단계마다 `count/unit` 개의 눈금을 가운데 정렬로 세운다 — 폭이 곧 셀 수 있는 수량이다
(①·② 그대로). 단계 농도는 위에서 아래로 사다리를 타고, 단계 사이 전환율(%)이 주인공이라
점선 안내선 옆에 적는다. 캡션은 "한 눈금 = n건 · 다섯 눈금마다 긴 눈금".

⑪ **글리프 하나 = 정해진 수량** (`bar.texture:"pictogram"`). 사람·건물·차량·원 4종은
외부 아이콘 세트를 쓰지 않고 charts.js 안에 12×16 박스 path 로 직접 그린 것이다.
칸 어휘와 규칙이 같다 — 셀 수 없는 값이면 캡슐로 강등하고, 한 글리프가 몇인지는
렌더러가 산출해 "사람 하나 = 20명" 으로 적는다. 글리프 자체는 액센트 농도 사다리.

⑫ **순위 격자의 칸 숫자** (`bump.layout:"strip"`). 선이 엉키는 자리에서 순위를 칸 안
숫자로 직접 적는다. 칸 채움은 순위 사다리(1위가 가장 진함)이고, 글자색은 채움이
.55 이상이면 `--card` 로 반전한다(③ 의 반전 규칙 계승). 오른쪽 ▲▼ 변동만 up/down
의미색을 쓴다 — §10 이 인정한 hue 예외 그대로.
