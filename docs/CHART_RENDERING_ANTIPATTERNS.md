...
## CHART-AP-46: 셀 수 있는 값을 면적으로 그림 — 칸 어휘를 안 쓰거나 칸 단위를 지어냄 (v8.6.6 신설)

**증상**
건수·인원·의석·정책 개수처럼 *하나씩 셀 수 있는* 값이 매끈한 면적 막대로 그려진다.
독자는 "10 은 5의 두 배" 라는 길이 비율만 얻고 "그래서 몇 개인가" 는 축을 되짚어야
안다. 반대 방향의 회귀도 있다 — 비율(%)·지수·평균처럼 *나눈 값* 에 칸 질감을 씌워
"한 칸 = 0.2%" 같은 셀 수 없는 단위가 차트에 박히거나, 최대값이 커서 수백 칸이
노이즈가 된다.

**원인**
① 렌더러가 표현을 데이터가 아니라 type 이름으로만 결정 ② 작성 모델(LLM)이 `texture`·
`unit` 을 직접 지정할 수 있으면 지어낸 단위가 그대로 그려진다 ③ 칸 수 상한을 실제
칸 수와 무관한 고정값으로 두면 칸이 넓게 퍼지거나(v8.6.5 사용자 지적: "한 칸 = 2조원인데
칸이 너무 넓다") 반대로 수백 칸이 뭉갠다.

**Fix (v8.6.1 도입 · v8.6.5 보정)**
① `isCountable(values, unitLabel)` — 값이 전부 정수이고 최대 500 이하이며 단위에 `%`
가 없을 때만 칸 질감(tick/rung/dot), 아니면 캡슐. **렌더러가 판정한다.**
② `niceUnit(max, maxMarks)` 가 `{1,2,2.5,5}×10ⁿ` 중 칸 수가 상한 이하가 되는 최소
단위를 산출하고, countable 이면 정수로 클램프한다 (`Math.max(1, …)`) — "한 칸 = 0.2건"
차단. 상한은 tick 100 / dot 50 / rung 60.
③ 칸 사이 틈은 상한이 아니라 **가장 긴 막대의 실제 칸 수** 로 나눈다 (`avail / longest`).
④ 칸 질감을 쓴 차트는 `keyFooter` 로 **읽는 법 캡션 필수** — "한 칸 = 1조원 · 다섯
칸마다 긴 눈금". 캡션이 없으면 칸 어휘는 절반만 전달된다 (v8.6.6 에서 `dot_matrix`
누락분 보강).
⑤ 프롬프트는 `texture`·`unit` 을 *지정하지 않는다* 고 명시 — WRITE-AP-5(추정 단정)의
시각물 판이다.

**점검**: 새 렌더러가 값 축을 그린다면 "이 값은 셀 수 있는가" 를 먼저 묻고, 칸을 쓰면
캡션을 반드시 붙인다. 어휘 SSOT [MONO_THEME_GUIDE §10.1](MONO_THEME_GUIDE.md).

---

## CHART-AP-47: 위계를 hue 로 구분 — 액센트 농도 사다리·해치만 허용 (v8.6.6 신설)

**증상**
순위·구성의 위계를 서로 다른 *색상* 으로 나눈다 (1위 파랑, 2위 초록, 3위 주황). 13종
테마 위에서 팔레트가 테마와 충돌하고, 흑백 인쇄·색각 이상에서 순서가 사라진다.
가까운 클래스의 회귀 — 농도 사다리를 쓰긴 하는데 **단 간격이 좁아** 구별이 안 되는 것
(v8.6.5 사용자 지적: 도넛 링의 .78/.60/.44 / v8.6.6 검수: `dot_matrix` 의 .95 vs .85 로
100칸이 한 덩어리).

**원인**
① 참고 자료·외부 차트 라이브러리의 기본 팔레트를 그대로 가져옴 ② 사다리를 쓰더라도
조각 수가 적을 때 7단 사다리의 앞쪽 세 단만 쓰면 서로 붙는다 ③ 최저 단이 너무 옅으면
(`.12`) 라이트 테마 카드 배경 위에서 사라진다.

**Fix (v8.6.5)**
① 위계는 **테마 액센트(`--accent`) 하나의 농도** 로만. 7단 `[1,.78,.60,.44,.30,.22,.16]`
(순위) / 4단 `[1,.46,.28,.16]` (구성), 8개 이상은 선형. 상수는 charts.js `inkLadder`
위 `LADDER_MIN` **한 곳**.
② 조각이 적어 단이 붙는 곳은 *간격이 넓은 전용 사다리* 를 쓴다 — 도넛 링
`[.72,.42,.22,.12]`, `dot_matrix` `[.55,.34,.24,.18,.16]`.
③ hue 를 쓰는 곳은 세 가지뿐 — up/down 의미색, sankey 다색 팔레트(§10 이 인정한 예외:
흐름의 *출처* 구분이 그 차트의 본체), 해치 패턴의 명목 카테고리 구분.
④ 축·눈금·라벨·읽는 법 캡션은 `--text`/`--muted` 로 남긴다. §3.3(큰 숫자 액센트 금지)과
충돌하지 않는다 — 바뀌는 것은 채움이지 글자색이 아니다.

**점검**: 새 렌더러에서 두 번째 색을 꺼내고 싶어지면 그건 대개 위계다. 농도로 되는지
먼저 확인하고, 그래도 색이 필요하면 위 세 가지 중 어디에 해당하는지 적어라.

---

## CHART-AP-48: 한쪽만 둥근 막대를 반투명 도형 겹침으로 제작 — 겹친 부분만 두 배로 짙어짐 (v8.6.6 신설, 사용자 catch)

**증상**
좌우 대칭 막대(diverging_bar 찬반, pyramid 인구)의 **중앙 축 근처에만 짙은 사각 덩어리**
가 생긴다. 값이 작을수록 덩어리가 막대 전체를 차지해 길이 비교까지 방해한다.

**원인**
"바깥 끝만 둥글고 축 쪽은 직각" 을 만들려고 **캡슐 rect(rx=h/2) 위에 축쪽 사각 rect 를
겹쳐** 그렸다. 둘 다 `fill-opacity` 0.85~0.9 라 겹친 h×h 영역의 실효 불투명도가
`1-(1-.9)²=.99` 로 뛴다. 불투명(opacity 1) 이던 시절엔 안 보이던 결함이, 농도로 위계를
주는 어휘(CHART-AP-47)로 오면서 드러났다.

**Fix (v8.6.5)**
`flatEndBar(g, {x, y, w, h, dir, fill, opacity})` — 막대 하나를 **단일 path** 로 그린다.
`M x,y H tip A rx,h/2 0 0 sweep tip,y+h H x Z` (좌향은 sweep 반전). 폭이 `h/2` 보다
짧으면 둥근 끝의 rx 를 폭만큼만 눕혀 값이 부풀지 않게 한다.

**규칙**: **반투명 도형은 절대 겹쳐 쌓지 않는다.** 복합 모양이 필요하면 path 하나로
그리거나, 정 안 되면 `<g opacity>` 로 그룹 전체에 한 번만 불투명도를 준다.

---

## CHART-AP-49: type 전용 CSS 폭 캡이 viewBox 와 어긋남 — 카드 가운데 몰림 + 좌우 빈 여백 (v8.6.6 신설, 사용자 catch)

**증상**
특정 type 의 차트만 카드 가운데에 작게 몰려 그려지고 좌우가 휑하다. 렌더러의 `viewBox`
를 아무리 손봐도 안 고쳐진다 — 원인이 JS 가 아니라 CSS 라서 그 자리를 계속 못 찾는다
(도넛에서 여러 버전에 걸쳐 재발).

**원인**
`charts.css` 의 `.chart-stage-donut{ max-width: 320px; margin: 0 auto; }` — 도넛이
*원형만* 그리던 시절(v4.2.1)에 "모바일에서 너무 커지지 않게" 넣은 캡. 이후 렌더러가
우측 범례를 붙인 **가로형(W 560)** 으로 진화했는데 캡은 그대로라, SVG 가 320px 로 눌린
뒤 가운데 정렬돼 카드 좌우가 빈다. CSS 와 viewBox 가 서로 다른 시대의 가정을 들고
있었던 것.

**Fix (v8.6.5)**
해당 규칙 삭제. 스테이지는 다른 type 과 같이 `width:100%` + SVG **natural ratio** 만
따른다 (`.chart-stage` 공통 규칙, v4.2.1 주석이 이미 선언한 원칙이다).

**규칙**: **차트 스테이지에 type 전용 폭·높이·aspect-ratio 캡을 두지 않는다.** 크기는
렌더러의 `viewBox` 한 곳에서만 결정한다. 모바일 축소가 필요하면 `@media` 로 카드
padding·글자 크기를 줄이지, 스테이지를 자르지 않는다. "차트가 가운데 몰린다" 는 신고를
받으면 JS 보다 **CSS 의 type 별 규칙을 먼저 grep** 할 것.

---

## CHART-AP-50: 기준이 되는 눈금 척도를 값이 넘었다고 척도 자체를 늘림 (v8.7.0 신설, 선제)

**증상**
"눈금 100개 = 목표" 라고 약속한 게이지에서 값이 목표를 넘자 링을 120 눈금으로 늘려 그린다.
그러면 100% 짜리 링과 130% 짜리 링이 *같은 모양* 이 되고(둘 다 꽉 참), 옆에 놓인 다른
게이지와도 비교가 안 된다 — 척도가 차트마다 달라졌기 때문이다.

**원인**
"값이 안 들어가면 축을 늘린다" 는 일반 차트의 습관을 *척도가 곧 의미인* 차트에 그대로
적용한 것. line 의 y축은 늘려도 되지만(축 라벨이 새 범위를 말해준다), 게이지의 링은
"목표" 라는 뜻을 지고 있어서 늘리는 순간 그 뜻이 사라진다. dot_matrix 의 100칸,
donut 의 100 눈금 링도 같은 성격이다.

**Fix (v8.7.0)**
`drawGauge` 는 채움을 100 눈금으로 **클램프** 하고, 초과분(≤100)을 링 *바깥* 반지름
(R+10~R+17)에 두 번째 바퀴로 그린다. 읽는 법 캡션이 "목표 초과 18눈금" 으로 명시하고,
가운데 "달성 118%" 가 수치로 못을 박는다.

**규칙**: **뜻을 진 척도(100 눈금 링 · 100칸 와플 · 반원 게이지)는 값이 넘어도 늘리지
않는다.** 초과는 *바깥에 따로* 그리고 캡션·숫자로 말한다. 늘려도 되는 것은 뜻이 없는
축(line/bar 의 값 축)뿐이다.
