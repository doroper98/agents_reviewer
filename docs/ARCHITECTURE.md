...
- **Phase 2B** ([§9](../REFACTOR_V5_PLAN.md)) — Visualization Capability Registry. `docs/VISUAL_CAPABILITY_REGISTRY.yaml` (16종 type — safe 11 / guarded 3 / experimental 2 분포, Plan §9.3 정본) + `src/visual/capability_registry.py` (yaml 로더 + 3-tier 정책 enforcer + `assert_chart_in_registry`). VisualPlanner 의 emit 경로에 Registry 가드 통합 — AP-V5-27 (미등재 차트 emit 금지) 강제. experimental (chord / treemap) 은 ResearchDirector `must_have` 명시 시만 허용.
