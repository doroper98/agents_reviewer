...
## v8.7.0 — 차트 유형 3종 신설 · 표현 옵션 3종 (CHART_REDESIGN_V8_6 Phase 5)

참고 자료 2차 흡수분을 랜딩했다. 차트 type **31종 → 34종**, 기존 유형에 덮어쓰기 옵션 3종.

### 새 유형

- **`gauge` — 값 하나를 목표에 견준다.** 반원에 눈금 100개를 두고 그 100개가 곧 목표다.
  채워진 눈금이 달성률이고 남은 눈금이 갈 길이라, "71%" 라는 숫자보다 "29눈금 남았다" 가
  먼저 읽힌다. 목표를 넘으면 링을 늘리지 않고 초과분을 링 바깥에 한 바퀴 더 그린다
  (링을 늘리면 "링 전체 = 목표" 라는 약속이 깨진다). 지지율·달성률·가동률·접종률처럼
  *하나의 수치가 목표에 얼마나 닿았나* 가 논점일 때. 여러 항목이면 지금처럼 bullet.
- **`spectrum` — 두 극 사이 어디쯤인가.** 긴축↔완화, 개방↔보호, 규제↔자율 같은 축에
  국가·정당·기관을 앉힌다. 값이 수량이 아니라 *위치*(-1~+1)라 눈금도 값 라벨도 두지
  않고, 초점 하나만 큰 점으로 남긴다. 라벨이 몰리면 두 층으로 비껴 앉히고 연결선을 남겨
  어느 점인지 잃지 않는다.
- **`funnel` — 단계마다 줄어드는 흐름.** 지원→서류→면접→최종, 신청→심사→집행. 폭을
  면적으로 그리지 않고 눈금을 세워 만든다(눈금 하나가 정해진 수량). 단계 사이 전환율은
  코드가 계산해 적는다. 수가 *늘어나는* 단계가 있으면 감소 흐름이 아니므로 그리지 않는다
  (그건 분배 = sankey, 또는 비교 = bar).

### 새 표현 옵션 (안 쓰면 지금까지와 똑같이 그려진다)

- **`bump.layout:"strip"`** — 순위 선이 엉키는 자리에서 칸에 순위 숫자를 직접 적는
  격자. 1위가 가장 진하고, 오른쪽에 첫 시기 대비 ▲▼ 변동.
- **`stacked.texture:"rung"`** — 세로 칸 적층. 칸을 세면 합이 보인다. 이름이 6자를
  넘거나 항목이 8개를 넘으면 렌더러가 기본 가로 적층으로 되돌린다(판독 우선).
- **`bar.texture:"pictogram"` + `glyph`** — 사람·건물·차량·원 네 글리프를 세는 막대.
  글리프 하나가 몇인지는 렌더러가 산출해 "사람 하나 = 10명" 으로 적는다. 셀 수 없는
  값(비율·지수)이면 캡슐 막대로 되돌린다.

### 데이터가 맞으면 코드가 옮겨 앉힌다 (type-fit R8·R9)

- **R8** — 항목이 하나뿐인 막대인데 그 행이 목표(`target`)를 들고 있으면 gauge 로.
  목표가 없으면 손대지 않는다(목표를 지어내지 않는다).
- **R9** — 분기·합류 없는 사슬형 sankey 인데 노드마다 수가 실려 있고 그 수가 줄어들면
  funnel 로. 링크 값만으로는 첫 단계에 몇이 있었는지 알 수 없어서, 노드가 스스로 수를
  들고 있을 때만 옮긴다.

### 검토 후 채택하지 않은 것

- **평행좌표(parallel_coords)** — 시험 렌더 결과 한글 축 이름이 축 간격보다 길어 서로
  부딪히고(세로쓰기는 한글이 옆으로 누워 더 안 읽힌다), 축마다 눈금이 달라 선이 교차해도
  뜻이 없었다. 같은 질문은 격자(heatmap)·작은 배수(small_multiples)·spectrum 이 더
  정직하게 답한다. 판단 근거는 계획서 §9.2 에 남겼다.

### 검수

갤러리 47장을 midnight_indigo / editorial_cream / reportage_noturno 3테마로 렌더해
잘림·라벨 겹침·빈 프레임·읽는 법 캡션 누락·콘솔 경고를 기계로 훑어 **잔여 0건**. 시험
렌더 중 잡아 고친 것 3건(성향 축 라벨 겹침, 게이지 초과 눈금 간격, 세로 칸 적층의 열
간격·여백). 렌더 DOM 스냅샷은 **기존 41키 불변**, 신규 6키만 추가(`gauge`/`spectrum`/
`funnel`/`bump:strip`/`stacked:rung`/`bar:pictogram`).

---

## v8.6.6 — 차트 전수 검수 · 결함 7건 수정 · 안티패턴 4건 등재 (CHART_REDESIGN_V8_6 Phase 3·4)

갤러리 41장(31 type + 표현 변형 10)을 midnight_indigo / editorial_cream / reportage_noturno 3테마로 전부 헤드리스 렌더하고, **화면 좌표 기준** 자동 스캐너로 훑었다 — 캔버스 밖으로 나간 글자, 서로 겹친 라벨, 빈 프레임, 읽는 법 캡션 누락, 사다리 최저 단 소실. 나온 결함 7건을 전부 고쳐 **잔여 0건**이다.

### 고친 것

1. **주석 선의 설명이 카드 밖으로 잘려 나갔다** (캔들 "증권가 평균 목표", 산점도 "선진국 평균 성장률"). 가로 기준선(hline) 라벨을 우측 여백에 무조건 밀어 넣었는데, 라벨이 여백보다 길면 그대로 잘렸다. 이제 들어갈 때만 여백에 두고, 안 들어가면 선 위 오른쪽 끝에 안쪽 정렬한다.
2. **두 축 그래프의 왼쪽 눈금이 전부 "4"** 였다. 축 라벨 형식이 `.0f` 로 고정이라 3.5~4.0 구간의 금리 축이 다섯 칸 모두 같은 글자였다. 이제 눈금끼리 서로 구별되는 *최소* 소수 자리를 자동으로 고른다.
3. **끝 라벨이 축 눈금 위에 겹쳤다** ("1,400" 위에 "원/달러 1402"). 축 눈금을 라벨 배치 점유도에 등록하지 않았고, 후보가 다 막히면 무조건 첫 후보로 되돌아갔다. 축 눈금을 등록하고, 후보를 6개로 늘리고, 그래도 다 막히면 *가장 덜 겹치는* 자리를 고른다.
4. **세계 지도에서 이웃 나라 라벨이 포개졌다** (한국 ↔ 일본이 매번). 중심점에 그냥 찍던 것을, x 가 가까운 라벨끼리 세로로 밀어내고 옮긴 라벨에는 실낱 연결선을 붙였다.
5. **100칸 와플의 두 집단이 같은 색이었다.** 강조 집단이 0.95, 그다음이 0.85 라 사실상 구별이 안 돼 100칸이 한 덩어리로 보였다(도넛 링에서 지적받은 것과 같은 클래스). 간격이 넓은 사다리 `[.55,.34,.24,.18,.16]` 로 교체.
6. **그 와플에 읽는 법 캡션이 없었다.** "칸 하나가 정해진 수량" 어휘의 원형인데 정작 캡션이 빠져 있었다 — "점 하나 = 100 중 1 · 한 줄 = 10" 추가.
7. **지표 막대의 값이 항목 이름 위에 덮어 찍혔다** ("선물 - 현물" 위에 "−0.42p 백워데이션"). 막대가 길면 음수 값 라벨이 왼쪽 이름 칸까지 밀고 들어갔다. 자리가 있을 때만 바깥에 두고, 없으면 막대 끝 위로 올린다.

### 안티패턴 4건 등재 (누적 49개)

- **CHART-AP-46 셀 수 있는 값을 면적으로 그림** — 칸 어휘를 안 쓰거나 반대로 비율에 칸을 씌워 "한 칸 = 0.2%" 같은 단위를 만드는 것. Fix = 렌더러가 `isCountable` 로 판정, `niceUnit` 정수 클램프, 틈은 실제 칸 수 기준, 읽는 법 캡션 필수.
- **CHART-AP-47 위계를 hue 로 구분** — 액센트 농도 사다리만 허용. 단이 붙어 구별이 안 되는 경우도 같은 클래스로 묶었다. hue 예외는 오름/내림 의미색 · sankey 흐름 팔레트 · 해치 3곳뿐.
- **CHART-AP-48 한쪽만 둥근 막대를 반투명 도형 겹침으로 제작** (사용자 catch) — 겹친 부분만 농도가 두 배로 쌓여 중앙에 짙은 덩어리. Fix = 단일 도형(path). **반투명 도형은 겹쳐 쌓지 않는다**.
- **CHART-AP-49 type 전용 CSS 폭 캡이 viewBox 와 어긋남** (사용자 catch) — 도넛이 여러 버전에 걸쳐 카드 가운데로 몰리던 진짜 원인. Fix = 캡 삭제. **크기는 `viewBox` 한 곳에서만 정한다.** "차트가 가운데 몰린다" 신고를 받으면 JS 보다 CSS 의 type 별 규칙을 먼저 볼 것.

### 확인·회귀

`svg_prerender.B_PLAN_CHART_TYPES` 와 [report_bundle_v1.md §9](docs/CONTRACTS/report_bundle_v1.md) 에 신규 4종(treemap/tree/histogram/calendar_heat)이 등록돼 있음을 확인했다(2a/2b 랜딩분). DOM 스냅샷은 **7키 변경** — `candle` · `scatter`(기준선 라벨) · `dual_line`(축·끝라벨) · `choropleth`(라벨 밀어내기) · `dot_matrix`(사다리+캡션) · `indicator`(값 라벨) · `line`(끝 라벨 배치 헬퍼 공유분). 나머지 34키 불변.

---

## v8.6.5 — 위계 사다리를 테마 액센트 농도로 + 사용자 지적 5건

**이번 버전은 보이는 것을 바꾼다.** charts.js 를 직접 고쳤으므로 이미 발행된 보고서의 차트도 다음 열람부터 새 색으로 보인다. 데이터 계약은 한 글자도 바뀌지 않았다.

### 위계 사다리의 색 — 잉크에서 테마 액센트로 (사용자 지시 2026-09-03)

v8.6.1 이 참고 자료의 표현 문법을 흡수하면서 *색까지* 따라갔다. 참고 자료는 검정 잉크 한 색으로 위계를 표현하는데, 우리 보고서는 테마가 13종이라 그 회색 채움이 보고서 전체 색감과 겉돌았다. 이제 **데이터 자국은 `--accent` 를 깔고, 순위·구성의 위계는 그 액센트의 농도로만** 표현한다.

- **바뀐 것**: 막대 칸·캡슐·점·경로선·area 세로선·도넛 눈금 링·롤리팝 줄기·treemap/heatmap/calendar_heat 칸·tree 연결선·waterfall 합계·bullet 구간 띠·dot_matrix 채움·해치 패턴 선색.
- **안 바뀐 것**: 축·눈금·라벨·읽는 법 캡션·그리드(`--text`/`--muted`) · 오름/내림 의미색(캔들, 찬반 막대의 반대측, 폭포 감소) · sankey 의 다색 팔레트(mono guide 가 인정한 예외 — 흐름의 출처를 색으로 구분하는 것이 이 차트의 본체다) · 인구 피라미드 왼쪽 집단의 중립 톤 · 해치 패턴이 명목 카테고리를 가르는 규칙.
- **§3.3 과 충돌하지 않는다** — 바뀐 것은 채움이지 글자색이 아니다. 큰 숫자는 여전히 `--text` 고, 액센트 글자는 차트당 1요소다.
- 사다리의 최저 단을 `.12` → **`.16`** 으로 올렸다. 액센트가 옅은 라이트 테마(editorial_cream)에서 `.12` 는 카드 배경 위에서 사실상 사라졌다. 상수는 `inkLadder` 위 `LADDER_MIN` 한 곳에서만 관리한다.

### 함께 고친 5건 (전부 사용자 catch, `samples/chart_redesign_v8_6_compare.html` 리뷰)

1. **막대 칸이 너무 넓다 — "한 칸 = 2조원인데, 1조원이어야"**: 칸 수 상한이 48 이라 최대값 69 에 한 칸 = 2 가 잡혔고, 칸 사이 틈을 *상한* 으로 나눠서(폭/48) 실제 35칸뿐인 막대가 폭 전체로 퍼졌다. 상한을 100(점 질감은 50)으로 올려 값 100 이하면 한 칸 = 1 이 되게 하고, 틈은 **가장 긴 막대의 실제 칸 수** 로 나눈다. 세로 칸도 같은 원칙. 칸 단위는 정수로 클램프한다(칸 질감은 셀 수 있는 값 전용인데 작은 최대값에서 "한 칸 = 0.2건" 이 나오던 것).
2. **도넛이 카드 가운데에만 몰리고 좌우가 빈다 (고질적)**: 근본 원인은 `charts.css` 의 `.chart-stage-donut{ max-width: 320px }` — 원형만 그리던 v4.2.1 시절의 캡인데, 지금 도넛 SVG 는 우측 범례가 붙은 가로형(W 560)이라 320px 로 눌리고 가운데 정렬돼 좌우가 비었다. **캡을 삭제**해 다른 차트와 같이 카드 폭을 채운다. 함께 링을 키우고(반지름 96 → 118, 눈금 40% 길게, 중앙 숫자 42 → 48) 범례를 오른쪽으로 밀었다.
3. **도넛 조각 구별이 약하다**: 링 모드가 7단 사다리를 써서 비핵심 조각들이 .78/.60/.44 로 붙어 있었다. 링은 조각이 6개 이하이므로 **간격이 넓은 전용 사다리 `[.72, .42, .22, .12]`** 로 바꾸고 조각 경계 틈을 1칸 → 2칸으로 벌렸다.
4. **찬반 막대·인구 피라미드 중앙에 짙은 덩어리**: 바깥 끝만 둥글게 하려고 캡슐 사각형 위에 축쪽 사각형을 *겹쳐* 그렸는데 둘 다 반투명이라 겹친 영역만 농도가 두 배로 쌓였다. **막대 하나를 단일 도형(path)으로** 그려 겹침 자체를 없앴다(폭이 짧으면 둥근 끝을 그만큼만 눕혀 값을 부풀리지 않는다).
5. **면적 차트에서 선 아래 그라데이션이 사라졌다**: v8.6.1 이 그라데이션을 세로 실선으로 *대체* 하면서 면적의 크기를 눈으로 잡아주던 채움까지 없앴다. 이제 채움을 먼저 깔고 그 위에 세로 실선을 얹는다.

### 회귀

DOM 스냅샷 41키 중 **32키 변경** — 요소 수까지 바뀐 것은 위 5건의 대상인 `area`(그라데이션 추가) · `bar`(칸 69개) · `donut`(링 확대·틈) · `diverging_bar`(도형 반으로) · `pyramid`(도형 반으로) 뿐이고, 나머지 27키는 **색 속성만** 바뀌었다. 변경 없음 9키 = `bubble`(이미 액센트) · `candle`·`gantt`·`indicator`·`iv_skew`(의미색·UI) · `sankey`·`stakeholder_map`·`tree`·`dot_matrix`(뒤 둘은 요소 수만 비교하는 loose 대상이라 색 변화가 해시에 안 잡힌다).
...
사용자 catch — "일반 보고서의 본문에서 단락 구분이 없으니 너무 읽기가 어려워. 단락, 문단 구분이 적절히 이뤄지면 훨씬 읽기 좋을거 같아. 적절한 들여쓰기도 있으면 좋겠어."

발행본 실물에서 섹션 본문이 **문단 구분 없는 30줄 벽**으로 나왔다. 원인이 두 겹이라 하나만 고치면 재발한다.

- **① 렌더러가 `<p>` 를 안 만들었다** — `_format_structured_text` 가 문단 경계를 `<br><br>` 로만 냈다. `<p>` 요소가 하나도 안 생기니 CSS 의 `.freeform-prose p{margin:...}` / `.rep-prose p{...}` 가 **통째로 죽은 규칙**이었고, 같은 이유로 드롭캡(`has-dropcap` 의 `p:first-child::first-letter`)도 한 번도 작동한 적이 없었다.
- **② composer 가 `\n\n` 을 안 냈다** — SYSTEM_PROMPT 에 '한 문단 3~5문장' 규칙이 *이미 있었는데도* 지켜지지 않았다(다른 문체 규칙 사이에 한 줄로 묻혀 있었다).
- **Fix ①** `_as_paragraphs` — 문단 경계를 실제 `<p>` 로 감싼다. 단일 `<br>` 은 문단 안의 개행이라 유지, 블록 헤더(`<strong>제목</strong><br>본문`)는 한 문단으로 묶인다. 르포 템플릿도 같은 필터를 타므로 함께 고쳐진다.
- **Fix ②** `_split_long_paragraph` — 한 문단이 5문장을 넘으면 문장 경계에서 결정적으로 쪼갠다(목표 4문장). 경계 정규식이 `(?<=[.!?。？！])\s+` 라 종결 부호 *뒤가 공백*일 때만 매치 — `84.25달러` / `8.16%` / `3,180.55` 가 문장 끝으로 오인되지 않는다. 마지막이 한 문장뿐이면 앞 문단에 병합(고아 문단 방지). 프롬프트만 믿지 않는 이유가 ②다.
- **Fix ③** 프롬프트 강령 승격 — 3~5문장 / 6문장 초과 금지 / 화제 전환마다 새 문단 / **섹션당 최소 3문단**.
- **타이포그래피** — 문단 간격 `1em` + 첫 줄 들여쓰기 `1em`(사용자 요청). 첫 문단은 제목 바로 아래라 들여쓰지 않는다. `.contradiction-prose` 동일.
- **⚠ 구현 중 밟은 버그** — 고아 문단 병합을 `out[-2] = f"{out[-2]} {out.pop()}"` 로 썼더니 RHS 의 `pop()` 이 먼저 실행돼 리스트가 줄어든 뒤 대입 대상 `out[-2]` 가 계산되면서 **앞 문단을 통째로 덮어썼다**(문장 유실 + 다른 문단 복제). 문단 *수*만 세는 테스트로는 안 잡혀서, 회귀를 *문장 집합 보존*(유실 0 / 중복 0) 검사로 바꿔 잡았다.
- **회귀** `tests/regression/test_prose_paragraphs.py` 20종. 변이 6종 주입으로 non-vacuous 확인(위 버그 재주입 포함). 실제 Chromium 으로 모바일 폭(430px) 렌더 확인 — 벽 1개 → 문단 4개, 들여쓰기 15px, 문단 간격 15px.
