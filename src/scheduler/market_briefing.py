...
def _build_market_briefing_prompt(now_local: datetime, persona: str) -> str:
    ...
    request = (
        ...
        f"2. **[필수] KOSPI200 지수 선물·옵션 데이터를 적극적으로 웹 검색해 수집** — "
        f"증권사 데일리 파생 리포트, 연합인포맥스/이데일리/한국경제/파이낸셜뉴스의 "
        f"'선물·옵션 동향' 기사, KRX 정보데이터시스템을 찾아본다. 다음을 *주체별·행사가별* "
        f"로 최대한 확보: (a) 선물 최근월물 종가·등락·베이시스·미결제약정 증감, "
        f"(b) 선물 순매수를 외국인·금융투자·투신·연기금·은행·보험·개인 *주체별로 분해*, "
        f"(c) 프로그램 매매 차익/비차익, (d) VKOSPI 수준·방향, 풋/콜 비율, 변동성 스큐, "
        f"(e) 거래량·미결제·내재변동성(IV)이 두드러진 *주요 콜·풋 행사가*, "
        f"(f) 옵션 만기·동시만기·롤오버 일정과 맥스페인/매물벽 추정. "
        f"확보 못 한 항목은 '확인되지 않음' 으로 표시(추정 금지).\n"
        ...
        f"하나 포함** (제11 렌즈, 파생 데스크 전문가 수준). 이 섹션은 다음을 모두 다룬다: "
        ...
        f"관측 조건으로부터 시사점을 읽되, 데이터 미확인 항목도 개념·관전 포인트는 설명해 "
        f"섹션을 비우지 말 것.\n"
