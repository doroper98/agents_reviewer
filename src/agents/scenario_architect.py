...
SYSTEM_PROMPT = (
    ...
    "- 각 시나리오에 'confidence' (0.0~1.0) — 이 시나리오 판단의 신뢰도 종합치 (출처 다양성·시의성·전문가 합의 기반)\n"
    "- 각 시나리오에 'driver_signals' (최대 4개) — 이 시나리오로의 분기를 *현재 관측 가능한* 형태로 식별하는 선행 지표. 추상 표현 금지, 구체적 사건/수치 형태로\n"
    "- impact_by_player 의 impact 텍스트는 가능하면 정량 강도 단어 포함 ('극심한 타격', '높은 충격', '중간 영향', '낮은 파급' 등) — 시각화에서 magnitude 추출에 사용\n"
    ...
    '      "driver_signals": ["관측 가능한 선행 지표 1", "지표 2"],\n'
    '      "impact_by_player": [\n'
    '        {"player": "행위자명", "impact": "영향 (1줄, 정량 강도 단어 포함)", "sentiment": "positive|negative|neutral"}\n'
    "      ]\n"
    "    }\n"
    "  ],\n"
    '  "watch_signals": [\n'
