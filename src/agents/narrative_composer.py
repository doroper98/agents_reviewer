...
SYSTEM_PROMPT = (
    ...
    "             국가* 지원)→둥근 국기. logo=기관·기업의 *공식 도메인* (예: \"samsung.com\",\n"
    "             \"mofa.go.kr\")→원형 로고 — 확실한 공식 도메인만, 추측 금지 (실패 시 이니셜\n"
    "             fallback 이라 안전). kind:'person'=인물, photo=인물 공식 사진 URL (위키미디어\n"
...
_REPORTAGE_BLOCK = (
    ...
    "  으로 관계를 잇는다. 기관·기업·정부부처 노드엔 ``logo``(공식 도메인, 예 \"samsung.com\"\n"
    "  · \"whitehouse.gov\" — *확실한 공식 도메인만*, 추측 금지)를, 인물 노드엔 ``photo``\n"
    "  (위키미디어 등 *실존 확인된* 공식 사진 URL — 흑백 원형 렌더)를 채우면 카드 완성도가\n"
    "  올라간다. 둘 다 불확실하면 생략 — 로고는 이니셜, 인물은 실루엣으로 안전하게 대체된다.\n"
    "  URL·도메인을 지어내지 말 것 (틀려도 fallback 이라 깨지진 않지만 무의미).\n"
