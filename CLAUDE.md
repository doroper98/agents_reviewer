...
- Visualization: d3 v7 SVG 차트 (composer-emitted inline data, **34종 type** (v8.7.0) — v5.3.0 FT/Economist 7종 + sankey + v7.0.0 bump/bullet/connected_scatter 3종 + v7.5.0 combo/diverging_bar/pyramid/dot_matrix 4종 + v8.6.2 treemap/tree 2종 + v8.6.3 histogram/calendar_heat 2종 + v8.7.0 gauge/spectrum/funnel 3종. v7.9.17 network 폐기 CHART-AP-36)
...
[docs/CHART_RENDERING_ANTIPATTERNS.md](docs/CHART_RENDERING_ANTIPATTERNS.md). **50개 패턴 누적** (v5.8.8 — CHART-AP-27 폭포수 부호 / 28 빈 차트 프레임 / 29 NaN 노출, 모두 결정적 가드로 차단. v7.0.1~2 — CHART-AP-30 곡선 보간 왜곡 / 31 시계열 데이터 듬성 emit / v7.5.1 — 32 sankey 라벨 수치 중복 표기 / v7.9.8 — 33 scatter 라벨 충돌 / 34 dot_matrix 좌측 쏠림 / v7.9.14 — 35 composer diverging_bar 지수 등락률 0 누락 / v7.9.17 — 36 network 관계도 포맷 폐기 / v8.0.0 — 37 stakeholder_map force/physics 레이아웃 금지[선제, network 교훈 상속] / v8.2.10 — 38 등록된 dict-데이터 차트 type 의 validate_chart_data 분기 누락→100% silent drop[사용자 catch — 르포 관계도가 v8.0.0 이래 미표시였던 근본 원인] / v8.2.13 — 39 대륙 간 스케일 지도를 평면 메르카토르로 렌더→빈 바다·구석 왜곡[사용자 catch — 경도 span>=100° 평면 지도를 globe 로 결정적 자동 격상] / v8.2.16 — 40 stakeholder_map 엣지 라벨이 가운데 칼럼 카드·다른 라벨 위에 찍혀 가림→카드+라벨 장애물 de-confliction(수직 우선 밀어내기+연결선)+선 스타일 범례[사용자 catch] / v8.2.17 — 41 stakeholder_map 교차 칼럼 엣지 세로 구간이 한 통로에 포개짐→칼럼 사이 gap 에 엣지별 세로 레인(bendX) 균등 분배 라우터+GAP/VSP 확대[사용자 catch] / v8.2.18 — 42 stakeholder_map 노드 자산 어휘 부족(미지원 국기 KR 등 silent 이니셜 강등+로고·인물 실사진 슬롯 부재)→ISO 전 국가 국기(flagcdn+인라인 KR 포함 7종 fallback)+logo(공식 도메인→원형 로고)+photo(흑백 원형 인물 사진), 원격 자산은 프리로드 성공 시에만 오버레이[사용자 catch] / 43 stakeholder_map 엣지가 카드 뒤 관통(교차 엣지 수평 구간·같은 칼럼 skip 수직 구간)+라벨이 타 엣지 선·교차점 위 안착→장애물 인지형 직교 라우터(가운데 칼럼 행 사이 수평 코리더+바깥 세로 레인 우회, 평행 겹침 0)+라벨 장애물에 엣지 세그먼트 포함[사용자 catch], 전부 사용자 catch. CHART-AP-29 는 v7.9.17 에서 소스(market_fetcher)·합류(orchestrator) 2단 NaN 봉 차단 추가 / v8.5.11 — 44 composer 프롬프트 스키마↔validator 가드 스키마 계약 불일치로 heatmap·stacked 100% silent drop[감사 발견 — 가드 양형 수용(heatmap)·렌더 계약 통일(stacked)+격자 렌더 분기+전 type 프롬프트-모양 라운드트립 parity 테스트 `test_prompt_guard_parity.py` 신설, CHART-AP-38 클래스 통째 차단] / v8.5.12 — 45 관측 불가능한 시각물 손실 층[감사 발견 — 템플릿 has_data 규칙 중복 소유(freeform 에 폐기 network 잔존·stakeholder_map 부재)·markers 0개면 지도 전체 미렌더·world-atlas CDN 단일 의존·파국 폴백의 charts 유실. 시각물 전달 보증으로 통합 대응: has_data SSOT `chart_renderable` 필터 + usage_log emit/kept 2단(기아↔배관이상 분리로 자기증폭 고리 절단) + 지도 렌더조건 완화 + world-atlas 로컬 폴백 + 폴백 차트 salvage] / v8.6.6 — **46 셀 수 있는 값을 면적으로 그림**(칸 어휘 미적용 또는 칸 단위 날조 → `isCountable`+`niceUnit` 렌더러 판정·정수 클램프·실제 칸 수 기준 틈·읽는 법 캡션 필수) / **47 위계를 hue 로 구분**(→ 테마 액센트 농도 사다리 1색, 단이 붙으면 간격 넓은 전용 사다리, hue 는 up/down·sankey·해치 3곳뿐) / **48 한쪽만 둥근 막대를 반투명 도형 겹침으로 제작**(겹친 부분만 두 배로 짙어짐 → `flatEndBar` 단일 path, 반투명 도형 겹쳐쌓기 금지)[사용자 catch] / **49 type 전용 CSS 폭 캡이 viewBox 와 어긋남**(`.chart-stage-donut{max-width}` → 카드 가운데 몰림·좌우 여백. 스테이지는 natural ratio 만, 크기는 viewBox 한 곳에서)[사용자 catch] / v8.7.0 — **50 뜻을 진 척도를 값이 넘었다고 늘림**(게이지 100 눈금 링·100칸 와플 → 100% 와 130% 가 같은 그림이 됨. 채움은 클램프하고 초과분만 링 바깥에 별도 + 캡션·숫자 명시)[선제]):
...
SSOT: [docs/REPORT_WRITING_ANTIPATTERNS.md](docs/REPORT_WRITING_ANTIPATTERNS.md). 28개 패턴 누적 (v5.8.8 — WRITE-AP-15 시장수치 자유서술[최우선] / 16 주장→사실 / 17 인과 과장 / 18 행사 혼동 / 19 일방서사 / 20 제목·본문 무게 / 21 신뢰도% 노출, 2026-06-03 일일 브리핑 회귀. v7.0.0 — WRITE-AP-22 기준시점 오선택. v7.6.4 — WRITE-AP-23 TTS 발음 표기 글 누수[사용자 catch]. v7.9.6 — WRITE-AP-24 고유명사 한글 음차 본문 누수[사용자 catch — 보고서와 영상 음성 내레이션 분리]. v8.2.4 — WRITE-AP-25 편집장 실패 1-섹션 폴백/절단본을 정상 완료로 무경고 발행[사용자 catch — deep 타임아웃→minimal fallback→degraded 플래그+텔레그램 경고+헤더 배너로 차단]. v8.2.9 — WRITE-AP-26 본문이 가리킨 시각물(아래 관계도/지도) 미표시[사용자 catch — 프롬프트 시각물-본문 일치 강제 + _reconcile_visual_references 안전망]. v8.5.7 — WRITE-AP-27 본문이 문단 구분 없는 한 덩어리[사용자 catch — 렌더러가 `<p>` 대신 `<br><br>` 만 내 CSS 문단 규칙·드롭캡이 죽어 있었고 composer 도 `\n\n` 미준수. `_as_paragraphs` 실 `<p>` 변환 + `_split_long_paragraph` 결정적 분할(5문장 초과 시, 수치 소수점 보존) + 프롬프트 강령 승격 + 문단 간격·첫 줄 들여쓰기]. v8.5.13 — WRITE-AP-28 괄호 없는 *문장형* 시각물 지시를 안전망이 통째로 놓침[사용자 catch — "아래 표는 …정리한 것이다" 뒤에 아무것도 없었다. v8.2.9 안전망이 괄호 안만 매칭 + 토큰에 "표" 부재 + 르포엔 표 렌더 채널 자체가 없음(fact_grid·embedded_blocks 미렌더·watch_signals 고정). 프롬프트 "아래 표" 금지 + 문장형 제거 신설, 단 조사로 메타문장(는/은/가/이=제거)과 사실문장(에서/를=보존+경고)을 갈라 본문 훼손 방지]):
...
- `src/templates/static/` — d3.v7.min.js / charts.js / maps.js / charts.css / maps.css / world-atlas-110m.js (보고서 dir 로 동기화. `report_synthesizer.STATIC_ASSETS` 가 SSOT. world-atlas 사본은 v8.5.12 — CDN 차단 시 지도 폴백, CHART-AP-45)
...
- **기본 표현은 데이터가 정한다 (v8.6.1, 소급).** 셀 수 있는 값(정수·최대 500 이하)은 막대를 면이 아니라 *칸* 으로 세고 한 칸의 뜻을 렌더러가 산출해 읽는 법 캡션으로 적는다. 비율·지수는 캡슐. 캔들 몸통 캡슐 · 도넛 100 눈금 링(≤6조각) · 막대류 캡슐 끝(diverging_bar/pyramid/gantt/bullet) · lollipop 순위 사다리 · line 일별 점(≤40, ISO 날짜, 주말 속빈) · area 세로 실선(≤120) · scatter 추선(≤20) · range_bar 구슬 덤벨 · heatmap 둥근 칸(≤60). 조건을 벗어나면 옛 표현 유지. 덮어쓰기 옵션 `texture/unit/orientation/prior/marks/fill/mode/cells` + **v8.7.0 의 `bar.texture:"pictogram"`·`bar.glyph`(사람·건물·차량·원 4종, 글리프 하나 = unit) · `bump.layout:"strip"`(순위 격자) · `stacked.texture:"rung"`(세로 칸 적층, §2.7 한글 라벨 게이트)** 는 전부 선택이고 계약 밖 값은 drop(`schemas.validate_chart_options`) — 단 세로 칸의 한글 라벨 게이트만 렌더러가 가로로 강등한다. 어휘 SSOT [MONO_THEME_GUIDE §10.1](docs/MONO_THEME_GUIDE.md), 계획 SSOT [CHART_REDESIGN_V8_6_PLAN §4](docs/CHART_REDESIGN_V8_6_PLAN.md), 전·후 비교 [samples/chart_redesign_v8_6_compare.html](samples/chart_redesign_v8_6_compare.html).
- **위계는 테마 액센트 농도로 (v8.6.5, 소급).** 데이터 마크(막대 칸·캡슐·점·경로선·area 세로선·도넛 눈금 링·롤리팝 줄기·treemap/heatmap/calendar_heat 칸·tree 연결선·waterfall 합계·bullet 구간 띠·dot_matrix 채움)는 `--text` 가 아니라 **`--accent`** 를 깔고 7단 사다리 `[1,.78,.60,.44,.30,.22,.16]` 를 불투명도로만 쓴다 (참고 자료의 검정 잉크 룩이 13종 테마와 겉돈다는 사용자 지적). 축·눈금·라벨·읽는 법 캡션·그리드는 `--text`/`--muted` 그대로이고, up/down 의미색·sankey 다색 팔레트·pyramid 좌측 중립 집단은 불변 — §3.3 의 '큰 숫자에 액센트 금지' 와 충돌하지 않는다(바뀌는 것은 채움이지 글자색이 아니다). 최저 단 `.16` 상수는 charts.js `LADDER_MIN` 한 곳. 어휘 SSOT [MONO_THEME_GUIDE §10.1](docs/MONO_THEME_GUIDE.md).
...
- **34종 type** (v8.7.0 — gauge/spectrum/funnel 추가. v7.9.17 network 폐기 CHART-AP-36):
  ...
  - v8.6.2 신규 2종 (**guarded** tier — 위계, CHART_REDESIGN_V8_6_PLAN §5.1/§5.2): treemap (2층 구성 — 부문 → 세부. 1층 묶음 2~8 · 잎 3~40. stacked 는 1차원, donut 은 1층·≤8 조각) / tree (소속 위계 — 지배구조·계열사·조직도·정책 체계. 깊이 ≤3 · 노드 4~40. stakeholder_map 이 *관계* 라면 이쪽은 *소속*). treemap 은 registry 에서 experimental → guarded 승격 (렌더러 실장)
  - v8.7.0 신규 3종 (**guarded** tier — 2차 흡수, CHART_REDESIGN_V8_6_PLAN §9): gauge (단일 KPI 반원 100 눈금 링 — 지지율·달성률·가동률. `{value, target, label?, unit_label?}` · 채운 눈금 = 달성률 · 목표 초과분은 링 바깥에 별도 · 중앙 큰 숫자는 `--text`(§3.3) · 여러 항목이면 bullet) / spectrum (양극 축 성향 배치 — 긴축↔완화·개방↔보호에 행위자를 앉힘. `{rows:[{left_label, right_label, points:[{label, value:-1..1, emphasis?}]}]}` 2~6행 · 행당 점 ≤5 · **값은 수량이 아니라 위치** — 수량이면 bar/diverging_bar) / funnel (단계 감소 — 지원→서류→면접→최종. `[{stage, count}]` 2~6 단계 · 뒤 단계가 앞 단계보다 크면 emit X · 폭은 눈금 수가 만들고 단계 사이 전환율을 렌더러가 계산 · N→M 분배는 sankey). gauge·funnel 은 `usage_log.DATA_GATED_TYPES` — 목표·단계별 수가 없는 보고서에 재균형 힌트가 가면 날조 유도라 힌트 대상에서 제외 (calendar_heat 와 같은 이유)
  - v8.6.3 신규 2종 (**guarded** tier — 분포·달력, CHART_REDESIGN_V8_6_PLAN §5.3/§5.4): histogram (순서 있는 구간의 도수 — 연령대·금액대·기간대 × 건수·인원. 4~24 구간 · 합 > 0 · 세로 칸(rung) 열 · 최빈 구간만 잉크 채움 + 값 · 도수 0 은 짧은 대시. **출처에 이미 구간별 집계가 있을 때만** — 건별 원자료를 나눠 구간을 만들면 WRITE-AP-5. 구간에 순서가 없으면 bar) / calendar_heat (일별 강도 달력 — 주 열 × 요일 7행(월 위) · 5분위 농도 · 값 없는 날은 속빈 점 · 최대일 점선 링. 60~400행(초과 시 마지막 371일) · 날짜 중복 금지. 질문이 '추세' 가 아니라 '언제 몰렸나' 일 때 — 추세 자체면 line. `usage_log.DATA_GATED_TYPES` 라 재균형 힌트에서 제외 — 일별 데이터 없는 보고서에 힌트가 가면 날조 유도)
...
- **type-fit 파이프라인 (v8.6.4, `src/visual/type_fit.py`)** — 작성 모델이 데이터 모양에 안 맞는 type 을 골랐을 때 결정적 규칙으로 *맞는 type* 에 앉힌다 (0-LLM). 호출은 orchestrator 의 `_densify_ts_charts` **직후 · `_reconcile_visual_references` 직전**, flag `V8_TYPE_REFIT`(Config `enable_type_refit`, 기본 1 · `0` 이면 refit 이전과 byte-equal). 규칙: **R1** bar→histogram(라벨이 전부 순서 있는 구간 4~24개 · 값 0 이상 정수) / **R2** donut·stacked(시나리오 1개)→treemap(라벨이 `A · a1`/`A/a1`/`A > a1` 2층 · 그룹 2~8 · 그룹당 잎 ≥2 · 잎 6~40) / **R3** heatmap→calendar_heat(x 가 전부 ISO 날짜 60~400 · y 단일) / **R4** slope→range_bar `before_after`(items ≥8) / **R5** bar→세로 칸(표현만 — 항목 ≤8 · 라벨 ≤6자 · 셀 수 있는 값 · orientation·texture 미지정) / **R8** 단일 항목 bar 인데 그 행이 target 을 들고 있으면→gauge(목표 생성 금지 — target 없으면 no-op) / **R9** 사슬형 sankey(분기·합류 없음, 2~6 노드)인데 *모든 노드가 value 를 들고 있고* 수가 줄어들면→funnel(링크 값만으로는 첫 단계 수를 알 수 없어 노드 value 가 있을 때만) / **R6** line 명시적 no-op / **R7** 길이 0 gantt 는 `R7-pending` 로 계측만. **값·라벨 생성 금지 · 데이터 폐기 금지 · 변환 후 가드 실패면 원본 유지.** 변환된 섹션은 `ComposedSection.model_validate` 로 `_drop_invalid_charts` 를 한 번 더 탄다. 계측은 `usage_log` 의 `refit: [{from,to,rule}]` + `analyze()['refit_distribution']` (오배치 빈도가 프롬프트 결정 트리 보강 근거 — 5-Layer ① 과 같은 원리). 발행본 소급은 `scripts/patch_report.py --refit` (표현 변경이라 render_revision 소수부 +1). 코퍼스 실측: `python -m src.visual.type_fit --scan reports/ --dry-run` (읽기 전용). 프롬프트 1선은 결정 트리의 `[자주 나는 오배치 4가지]` 블록. SSOT: [CHART_REDESIGN_V8_6_PLAN §6](docs/CHART_REDESIGN_V8_6_PLAN.md), 회귀 `tests/regression/test_type_fit.py`.
- 신규 type 추가 절차: ① `charts.js` 의 `RENDERERS` dict 에 함수 추가 ② composer SYSTEM_PROMPT 의 type 별 data 스키마 섹션에 추가 ③ `src/visual/schemas.py` 의 `_TYPE_TO_GUARD` 에 가드 추가 ④ `docs/VISUAL_CAPABILITY_REGISTRY.yaml` 등록 ⑤ `src/visual/usage_log.py:KNOWN_CHART_TYPES` 추가 ⑥ `tests/regression/fixtures/chart_type_scenarios.yaml` 시나리오 추가 ⑦ 회귀 테스트 ⑧ dict-데이터 type 이면 `src/visual/schemas.py:_DICT_DATA_REQUIREMENTS` 등록 (템플릿 has_data SSOT — 두 archetype 공용, v8.5.12 CHART-AP-45) ⑨ `tests/regression/test_prompt_guard_parity.py:PROMPT_SHAPES` 에 *프롬프트가 문서화한 모양 그대로* fixture 추가 (누락 시 완전성 테스트 실패 — CHART-AP-44 클래스 차단) ⑩ `src/handoff/svg_prerender.py:B_PLAN_CHART_TYPES` 등록 (osint 렌더 게이트가 모르는 신규 type 은 `prerendered_svg` 폴백이 없으면 영상에서 *무경고 소실*) ⑪ [docs/CONTRACTS/report_bundle_v1.md §9](docs/CONTRACTS/report_bundle_v1.md) pin 줄 갱신 (additive 면 `schema_version` 유지) ⑫ `tests/regression/test_chart_correctness.py` 에 수용/거부 케이스 + `samples/chart_gallery_v7.html` fixture 추가 후 `python scripts/chart_dom_snapshot.py --out ...` 로 baseline 에 신규 키 기록 (기존 키는 불변이어야 — `--diff-report` 로 증명).
...
- **v8.6.3 — 일별 변동 강도 달력 자동 주입** (`orchestrator._ensure_calendar_heat`, `_ensure_time_series_chart` 직후 같은 `report_format != "reportage"` 분기, 0-LLM): 주제 우선 종목(`_topic_priority_key`)의 풀 카드 *바로 뒤* 같은 섹션에 그 종목의 `calendar_heat` 1개(`|close_t/close_{t-1}-1|×100`, NaN 봉 제외·마지막 371행) 를 결정적으로 삽입한다. 추세(line/candle)가 답하지 못하는 '언제 몰렸나' 담당. 60행 미만 / composer 가 이미 calendar_heat emit / `ChartCountLimits`(standard 4 · deep 5) 초과 / 붙일 풀 카드 없음 이면 no-op. flag `ENABLE_CALENDAR_HEAT_INJECT` (기본 1, `0` → v8.6.2 와 byte-equal). 회귀 `tests/regression/test_calendar_heat_inject.py`. SSOT: [CHART_REDESIGN_V8_6_PLAN §5.4](docs/CHART_REDESIGN_V8_6_PLAN.md).
...
- **소급 발행 (v8.5.4)** — 정시 스케줄러가 못 돈 날의 브리핑은 `python3 -m src.scheduler.market_briefing yesterday|YYYYMMDD [--no-send|--chat-id N|--mode|--force]` 로 나중에 생성 (`run_backfill_market_briefing`). 실데이터 fetch + `report_kind` 배지 + 고정 프롬프트 문구 모두 정시와 동일하고, 대상일≠발행일이면 프롬프트에 소급 발행 지시가 붙는다(대상일 이후 사건 혼입·시점 뭉개기 차단). 휴장일 기본 거부. 정시 경로는 byte-equal. 운영 명령 SSOT 는 [VM_DEPLOY_PLAYBOOK §3](docs/VM_DEPLOY_PLAYBOOK.md).
...
- **v8.5.12 (CHART-AP-45)** — 렌더 조건 완화: markers 0개여도 rings·regions·sea_labels·arcs 중 하나가 있으면 그린다 (그전엔 markers 필수라 사거리권만 있는 payload 가 빈 상자로 발행). world-atlas 는 CDN 우선 + `world-atlas-110m.js` 로컬 사본 폴백 (단일 의존 제거).
...
- **본문 문단(v8.5.7~8)**: 르포도 일반 보고서와 같은 `structured` 필터를 타므로 v8.5.7 의 실 `<p>` 변환 + 5문장 초과 결정적 분할이 그대로 적용된다 (그 전까지 `.rep-prose p` 규칙이 `<p>` 부재로 죽어 있었다). v8.5.8 에서 첫 줄 들여쓰기 1em 추가, v8.5.9 부터 첫 문단도 포함(사용자 결정 — 문단마다 일관되게. 유일한 예외는 드롭캡, float 첫 글자가 밀려 깨지므로). 작성 강령은 `_REPORTAGE_BLOCK` 의 '한 문단 다섯 문장 이내 / 섹션당 문단 서넛 이상' 이 이미 일반보다 강하다.
