...
- **본문 문단(v8.5.7~8)**: 르포도 일반 보고서와 같은 `structured` 필터를 타므로 v8.5.7 의 실 `<p>` 변환 + 5문장 초과 결정적 분할이 그대로 적용된다 (그 전까지 `.rep-prose p` 규칙이 `<p>` 부재로 죽어 있었다). v8.5.8 에서 첫 줄 들여쓰기 1em 추가(첫 문단 제외). 작성 강령은 `_REPORTAGE_BLOCK` 의 '한 문단 다섯 문장 이내 / 섹션당 문단 서넛 이상' 이 이미 일반보다 강하다.
