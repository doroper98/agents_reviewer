...
- **로드맵**: Phase 0(트리거+directive+5막)·Phase 1(`stakeholder_map` 차트 + force 금지 CHART-AP-36) **완료**. Phase 1.5(전용 테마/폰트/플랫/어투/에필로그제거/배지) **완료**. 남은 것 — 르포 차트(stakeholder_map/sankey/globe) ambient 애니메이션 + 상시 차트 최소화 + 텔레그램 전용 후속 르포 신호. Phase 2 — ReportBundle 정합(osint 영상). Phase 3 — VM e2e.
